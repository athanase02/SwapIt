...
if (getenv('DB_HOST')) {
    ...
    // Check if using PostgreSQL (Render default)
    if (getenv('DATABASE_URL') && strpos(getenv('DATABASE_URL'), 'postgres://') !== false) {
        error_log("SwapIt: PostgreSQL detected but MySQL required. Please use MySQL add-on on Render.");
        // For now, fall back to localhost for development
        $host = "localhost";
        $username = "root"; 
        $password = ""; 
        $database = "SI2025";
...
function ensureTablesExist($conn) {
    try {
        ...
        if (!$result || $result->num_rows == 0) {
            error_log("SwapIt: Creating essential database tables...");
            ...
            if (!$conn->query($usersTable)) {
                error_log("SwapIt: Failed to create users table: " . $conn->error);
                return false;
            }
            ...
            if (!$conn->query($profilesTable)) {
                error_log("SwapIt: Failed to create profiles table: " . $conn->error);
                return false;
            }
            
            error_log("SwapIt: Essential database tables created successfully");
        }
        return true;
    } catch (Exception $e) {
        error_log("SwapIt: Error creating tables: " . $e->getMessage());
        return false;
    }
}
...
try {
    ...
    error_log("SwapIt: Database initialization error: " . $e->getMessage());
}
?>
